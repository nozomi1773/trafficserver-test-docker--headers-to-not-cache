...
# Test 2 - 3 :  Not Write Cache
#   Characteristic Client Request header : Cache-Control:no-store ( change CONFIG : CONFIG proxy.config.http.cache.ignore_client_no_cache 0 )
#   Characteristic Origin Response header : Cache-Control:max-age=5
#
# Via Infomation : [uScMsSf pSeN:t cCMp sS]
#   client-info is S(simple request, not conditional) , cache-lookup is M(miss) , server-info is S(served) ,
...
#   Characteristic Client Request header : Cache-Control:no-cache ( change CONFIG : CONFIG proxy.config.http.cache.ignore_client_no_cache 0 )
#   Characteristic Origin Response header : Cache-Control:max-age=5
#
# Via Infomation : [uScMsSfWpSeN:t cCMp sS]
#   client-info is S(simple request, not conditional) , cache-lookup is M(miss) , server-info is S(served) ,
...
tr.Processes.Default.Streams.stdout = "gold/200_OK_cache_write.gold"
tr.StillRunningAfter = ts
# Test 2 - 4 - 2 :  Cache Update ( revalidate )
#   Characteristic Client Request header : Cache-Control:no-cache ( change CONFIG : CONFIG proxy.config.http.cache.ignore_client_no_cache 0 )
#   Characteristic Origin Response header : Cache-Control:max-age=5
#   ( 2nd Request same URL )
#
